...
public class BrestTechParkApplication {
    public static void main(String[] args) {
        SpringApplication.run(BrestTechParkApplication.class, args);
    }
}

// Configuration для инициализации данных
@Configuration
class DataInitializationConfig {
    ...
    public org.springframework.boot.CommandLineRunner initData(
        ...
        return args -> {
            try {
                // Создаем типы помещений сначала
...
class WebController {
    ...
    public String dashboard(Model model, Authentication authentication) {
        ...
        if (userOpt.isPresent()) {
            ...
            } else if (user.getRole() == UserRole.RESIDENT) {
...
class ApiController {
    ...
    @PostMapping("/admin/reject-booking/{bookingId}")
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> authz
                        ...
                        .requestMatchers("/", "/login", "/register", "/booking",
                                "/api/register", "/api/announcements", "/api/room-types",
                                "/api/equipment-types", "/api/room-bookings/**", "/api/booking",
